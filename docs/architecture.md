...
码住采用本地优先架构。Android App 负责低摩擦收藏，Supabase 负责跨设备数据同步，CLI 和 Codex Skill 负责知识库能力。
...
Supabase 提供认证、PostgreSQL 数据库和 RLS。核心表：
...
CLI 是桌面端知识库入口。它不需要自建服务器，直接访问 Supabase API。

主要命令：

- `login`：保存 Supabase 登录会话。
- `collections`：列出收藏夹。
- `search`：按标题、公众号、收藏夹、摘要和 topics 检索。
- `read`：读取单篇文章全文。
